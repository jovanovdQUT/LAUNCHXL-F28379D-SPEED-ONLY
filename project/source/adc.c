...
// Nominal parameters
float IQ_REF = 0.5;
float KpoD = 5.0/*0.03*/, KioD = 0.1/*0.01*/;
float KpoQ = 5.0/*0.03*/, KioQ = 0.1/*0.01*/;
...
float temp = 0.0,  temp1 = 0.0, modul = 0.2, tempTheta = 0.0, tempAngle = 0.0, tempAngleE = 0.0;
...
//            }
            ...
            idPID.term.Ref = 0.0;
            idPID.term.Fbk = pi.d;
            idPID.param.Umax = adc_read.vdc; // VDC_MAX;
            idPID.param.Umin = -adc_read.vdc; // -VDC_MAX;
            idPID.param.Kp = KpoD; //  / adc_read.vdc;
            ...
            iqPID.param.Kp = KpoQ; //  / adc_read.vdc;
            iqPID.param.Ki = KioQ * DELTA_T; // iqPID.param.Kp * Kio;
            PID_GRANDO_F_FUNC(&iqPID);


            // Inverse Park

            ipv.d = idPID.term.Out;
            ipv.q = iqPID.term.Out;
            ipv.z = 0.0;            // pv.zero;
